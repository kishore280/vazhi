...
## Usage dashboard (JSON-path aggregation in SQL)

- **You can query *inside* a JSON column in Postgres, without pulling the whole thing into Python first.** `AgentRun.token_usage["total"]["total_tokens"]` (SQLAlchemy's JSON path indexing) becomes real Postgres JSON-path syntax under the hood — the database does the extraction, not your application code looping over rows.
- **`func.coalesce(..., 0)` guards against `NULL` breaking an aggregate.** If some rows have no `token_usage` data yet (a run that hasn't finished), summing over `NULL` would silently produce a `NULL` total instead of a real number. Coalescing to `0` first means every row contributes a real number to the sum, even ones with missing data.
- **This dashboard only worked because an earlier piece (`TokenUsageMiddleware`) already wrote data in the exact shape this query expects.** No coincidence — both were built against the same real vazhi reference, so the shapes matched automatically. This is the payoff of building faithfully to a spec even when you can't yet see how a later piece will consume it.

## Message feedback (composing independently-built features)

- **A later feature reusing an earlier one, unmodified, is a real signal the earlier one was built correctly.** `feedback_service.py` calls `submit_user_feedback_score()` — a function from `langfuse_service.py`, built session's earlier for a completely different purpose (tracing). No changes needed to either file for them to work together. This only happens when both were built against clean, well-defined boundaries rather than tightly coupled to their original use case.
- **App-level uniqueness, not a database constraint.** "One feedback per (message, user)" is enforced by a `SELECT` check before inserting, not a `UniqueConstraint` on the table. This is a real, if small, trade-off: a genuine race (two simultaneous requests) could both pass the check before either inserts — a DB-level constraint would be airtight, this isn't. Real vazhi's own comment acknowledges this explicitly rather than hiding it; sometimes "good enough, documented" beats "add a lock/constraint for a case that's very unlikely to matter."
- **Fire-and-forget side effects shouldn't be able to break the main operation.** Submitting the Langfuse score is wrapped in its own `try/except` — if Langfuse is down or errors, the feedback itself (already committed to Postgres) is unaffected; only a warning gets logged. The core action succeeding doesn't depend on every downstream side effect succeeding too.

## MCP (Model Context Protocol) tool loading

- **Transport allowlisting is a real security boundary, not an arbitrary restriction.** Only `sse`/`streamable_http` (both network transports) are allowed — `stdio` (spawning a local subprocess) is deliberately excluded. An agent that can configure its own tool servers shouldn't also be able to make the server spawn arbitrary local processes on its behalf — that would turn "add a tool" into "run any command," a huge escalation. This is the same instinct as the sandbox path-traversal checks: user-influenced config needs its own boundary, separate from what a trusted admin could configure.
- **Cache key includes a hash of the config, not just the server name.** `cache_key = f"{server_slug}:{config_hash}"` — if a server's config changes (different URL, different headers), the hash changes, so it's automatically treated as a cache miss and refetched, without needing to remember to explicitly invalidate anything when config changes. Stale cache entries under the old config hash get cleaned up right after ("stale_keys... key != cache_key").
- **Tool names get namespaced to avoid collisions**: `mcp__{server}__{tool}` (e.g. `mcp__github__createIssue`). Two different MCP servers could both expose a tool literally called `search` — without namespacing, the agent (and your own code) couldn't tell them apart or know which one actually got called.
- **`except ExceptionGroup`** — a newer Python construct (3.11+) for catching exceptions raised by concurrent task groups, where multiple sub-tasks might each fail independently and Python bundles all their exceptions together into one object instead of just propagating the first one it sees.

## Object storage (S3-compatible, for attachments)

- **A reference project's dependency can go stale between when it was written and when you build against it.** Real vazhi's own code uses MinIO — but MinIO Community Edition was archived in Feb 2026 (license changed to AGPL, binaries stopped, company pivoted to a paid product). Always worth checking whether a dependency is still actively maintained before building on it, especially for infrastructure pieces, not just trusting "the reference project uses X" as proof X is still a good choice today.
- **The `minio` Python package name is just an S3 API client — it doesn't require you to run MinIO's own server.** S3-compatible object storage means many different server implementations (MinIO, RustFS, SeaweedFS, actual AWS S3) all speak the same wire protocol. You can point the exact same client code at a completely different server just by changing the connection endpoint — zero code changes, only infra changes. This is the practical value of "S3-compatible": swap the backend without touching a single line of application code.
- **When a reference's specific tool goes stale, look at what the community actually migrated to** — not just "any alternative that exists." Several options existed (RustFS, SeaweedFS, Garage), but checking what people were *actually adopting* as the practical drop-in (same ports, same API surface, least migration friction) mattered more than picking whichever had the most GitHub stars or the most "mature" reputation in the abstract.

## Langfuse tracing (observability)

- **Graceful no-op by design, not by accident.** `is_langfuse_enabled()` checks for real credentials before doing anything; every function that touches the Langfuse client checks `if client is None: return` first. This means the tracing code can ship and run in every environment — dev, prod, a laptop with no API keys at all — without ever crashing or needing a feature flag to disable it. The "off" state is the natural fallthrough of the same code path, not a separate branch.
- `@lru_cache(maxsize=1)` on `get_langfuse_client()` — the client only gets constructed once per process, cached after that. Cheap to call this function everywhere you need the client; it's not rebuilding a network client on every call.
- **A trace ID gets created before the LLM call happens**, not after — `client.create_trace_id(seed=request_id)` is deterministic from a seed you already have (the request's own ID), so the trace ID exists before any API call, letting you record it (e.g. on the saved message) even if you never need to look anything up from Langfuse afterward.
- **`CallbackHandler` is how LangChain actually reports to Langfuse** — pass it into the model call's `config={"callbacks": [handler]}`, and every step LangChain takes (model call, tool call, retries) gets automatically reported, without manually instrumenting each one.
- **Config-source consistency matters, even when the reference doesn't have it.** Real vazhi's own `langfuse_service.py` reads credentials via raw `os.getenv(...)`, inconsistent with the rest of its codebase (which uses a centralized `Settings` object everywhere else). Deliberately deviated here — added the same fields to `Settings` instead — because matching a reference's *inconsistency* isn't the same as matching its *design*. Worth deciding case-by-case whether an inconsistency in the reference is "how they do it" (a design choice worth copying) or just an oversight (not worth propagating).
...
- **Python gotcha: `bool` is a subclass of `int`.** `isinstance(True, int)` is `True` in Python — so a naive "is this an int?" check would silently accept a boolean and treat it as `0`/`1`, corrupting a token count. `_safe_int()` explicitly checks `isinstance(value, bool)` *first* and rejects it before checking `isinstance(value, int)` — a real, easy-to-miss trap whenever you're coercing arbitrary values to numbers.
- **Prompt-cache hit detection**: providers that support prompt caching (reusing a previously-sent prefix instead of reprocessing it) report how many input tokens were served from cache, buried in `usage.input_token_details.cache_read` (or provider-specific variants). The middleware reads this to compute a cache-hit ratio — useful for noticing "this conversation's system prompt is being recomputed every single call" as a real cost problem, not just a vague guess.
...
- **Naming mismatch on purpose**: your own database stores `role` as `"user"`/`"assistant"`/`"system"`, but LangChain's own message classes and most frontend chat libraries expect `"human"`/`"ai"`/`"system"`. `_ROLE_TO_TYPE = {"user": "human", "assistant": "ai", "system": "system"}` is a small translation table bridging your storage vocabulary to the vocabulary the consuming library expects — a reminder that your database schema's naming doesn't have to match every downstream library's naming, just be translatable to it.
- **Ambiguous 404, deliberately**: `if conversation is None or conversation.uid != str(current_uid): raise HTTPException(404, ...)` — both "this thread doesn't exist" and "this thread exists but belongs to someone else" return the exact same 404. This is intentional: returning a different error for "exists but not yours" would let an attacker probe which thread IDs are real, even without ever seeing their content. Same status code for both cases means no information leaks either way.
...
- **`interrupt_on`'s `"when"` key takes a callable, not just a bool.** `HumanInTheLoopMiddleware(interrupt_on={"write_file": {"when": some_function}})` — approval isn't just "always require it for this tool," it can be conditional per *call*, based on that call's actual arguments. `_project_write_requires_approval` builds a closure that checks the specific `file_path` argument against the current project root at the moment of the call — writes inside the workdir skip approval, writes outside it don't.
- **Path-traversal defense pattern**: `_normalize_runtime_path` rejects a path if it contains `".."` (directory traversal), a backslash (Windows-style path sneaking into a POSIX-only system), or `"://"` (a URL disguised as a file path). Three different disguises for "this isn't really a safe absolute file path," all checked before ever trusting the string for a security-relevant decision.
...
## Python language niche used in the worker loop

- **`for...else` — the `else` runs only if the loop finished without `break`.** Confusing name (it's not "if the for loop had zero iterations," it's "if nothing broke out of it"). Used in `run_worker.py`: `for mode, chunk in agent.astream(...): ... if cancelled: break ... else: status = "completed"`. This means `status = "completed"` only ever gets set if the stream ran to natural completion — if a `break` fired (cancel requested), `status` keeps whatever it was already set to (`"cancelled"`) and the `else` block is skipped entirely. Cleaner than a separate `did_break` flag variable to track the same thing.
